...
const FuncInfoForm: React.FC<any> = (props) => {
    ...
    const modifyFunc = (e: any) => {
        if (merchantMenu == false) {
            ...
            Modal.confirm({
                title: '温馨提示',
                content: '确定要更改功能定义吗?该更改提交后生效',
                cancelText: '取消',
                okText: '确定',
                onOk: async () => {
                    try {
                        const values = await form.validateFields();
                        const update = { ...func, ...values };
                        if (props.onFuncChanged) {
                            props.onFuncChanged(update);
                        }
                    } catch (errorInfo) { }
                },
            });
    ...
    const doDeleteFunc = (_e: any) => {
        if (merchantMenu == false) {
            ...
            Modal.confirm({
                title: '温馨提示',
                content: '确定要删除功能[' + func.funCode + ':' + func.funName + ']定义吗?该更改提交后生效',
                cancelText: '取消',
                okText: '确定',
                onOk: async () => {
                    const values = await form.validateFields();
                    const delFunc = { ...func, ...values };
                    if (props.onDeleteFunc)
                        props.onDeleteFunc(delFunc);
                },
            });
        }
    };
